...
vector<lower=0>[2] sigma_geo; // maximum covariance due to geographic location
}
...
A[1,1] = -exp(alpha_auto[1]); // autoregressive effect of var 1 on itself
A[2,2] = -exp(alpha_auto[2]); // autoregressive effect of var 2 on itself
...
model{
  ...
  for (i in 1:N) {
    y[i,1] ~ ordered_logistic( eta[i,1] + geo_v[i,1], c1 );
    y[i,2] ~ ordered_logistic( eta[i,2] + geo_v[i,2], c2 );
  }
}
